...
class image {
	...
	public function resize($dst_w, $dst_h) {
		...
		$params = ['dst_image' => $this->dst_image, 'src_image' => $this->src_image, 'dst_x' => 0, 'dst_y' => 0, 'src_x' => 0, 'src_y' => 0, 'dst_w' => $dst_w, 'dst_h' => $dst_h, 'src_w' => $this->src_w, 'src_h' => $this->src_h];
	...
	public function crop($dst_w, $dst_h, $src_x, $src_y) {
		...
		$this->dst_image = imagecreatetruecolor($dst_w, $dst_h);
		$params = ['dst_image' => $this->dst_image, 'src_image' => $this->src_image, 'dst_x' => 0, 'dst_y' => 0, 'src_x' => $src_x, 'src_y' => $src_y, 'dst_w' => $dst_w, 'dst_h' => $dst_h, 'src_w' => $dst_w, 'src_h' => $dst_h];
		call_user_func_array('imagecopyresampled', array_values($params));

		return $this;
	}

	public function palette($percentages = [0.2, 0.7, 0.5]) {
		...
		return [strtoupper($r[0].$g[0].$b[0]), strtoupper($r[1].$g[1].$b[1]), strtoupper($r[2].$g[2].$b[2]), strtoupper($r[3].$g[3].$b[3]), strtoupper($r[4].$g[4].$b[4])];
	}
