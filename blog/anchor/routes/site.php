...
if ($home_page->id != $posts_page->id) {
	Route::get(['/', $home_page->slug], function () use ($home_page) {
...
*/
Route::get(['category/(:any)', 'category/(:any)/(:num)'], function ($slug = '', $offset = 1) use ($posts_page) {
 ...
 * Search
*/
Route::get(['search', 'search/(:any)', 'search/(:any)/(:any)', 'search/(:any)/(:any)/(:num)'], function ($whatSearching = 'all', $slug = '', $offset = 1) {
	// mock search page
	$page = new Page();
	$page->id = 0;
	$page->title = 'Search';
